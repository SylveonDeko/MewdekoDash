...
<script lang="ts">
  ...
  function updateVisibleItems(index: number) {
    ...
    for (
      let i = Math.max(0, index - lazyLoadBuffer);
      i <= Math.min(itemCount - 1, index + lazyLoadBuffer);
      i++
    ) {
      visibleItems.add(i);
    }
...
<div class="relative h-full w-full">
  {#if itemCount > 1}
    <ol
      class="flex items-center overflow-x-auto snap-x snap-mandatory no-scrollbar {$reducedMotion
        ? ''
        : 'scroll-smooth'}"
      bind:this={carousel}
    ...
    >
      {#each items as { props, component }, index}
        <li
          ...
          class="relative h-full w-full flex-shrink-0 flex-grow-0 basis-full snap-start"
        >
        ...
        {#if defaultAbsoluteNavigation}
          ...
          >
            {#each items as _, index}
              <button
                title="scroll #{index + 1} into view"
