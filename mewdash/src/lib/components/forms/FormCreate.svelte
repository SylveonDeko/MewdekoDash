...
<script lang="ts">
  ...
  $effect(() => {
    const formTypeMeta = FORM_TYPES.find(ft => ft.type === formType);
    if (formTypeMeta) {
      allowExternalUsers = formTypeMeta.allowsExternalUsers;

      // Clear options that don't make sense for external forms
      if (formType !== "Regular") {
...
<div class="space-y-6">
  ...
  >
      ...
      <div class="space-y-4">
        {#each questions as question, index (question.id)}
          <div
            ...
            role="listitem"
            class="backdrop-blur-xs rounded-lg border p-4 transition-all {draggedQuestionIndex === index ? 'opacity-50' : 'hover:shadow-md'}"
            ...
            <div class="flex items-start gap-3 mb-3">
              ...
              <div class="flex-1 min-w-0">
                <input
                  ...
                  oninput={(e) => updateQuestion(index, { questionText: e.currentTarget.value })}
    ...
    <div class="mt-6">
      <button
        class="group w-full py-4 rounded-lg font-medium transition-all hover:scale-[1.02] relative overflow-hidden"
