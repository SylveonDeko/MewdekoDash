...
<details class="group list-inside bg-mewd-dark-grey rounded-lg break-normal pb-4">
    <summary class="flex flex-row justify-start self-center w-full py-3 px-4 cursor-pointer">
        ...
        <p class="group-open:hidden px-4 text-sm whitespace-nowrap text-ellipsis overflow-hidden prose prose-invert prose-code:bg-gray-700 before:prose-code:content-[''] after:prose-code:content-[''] prose-code:rounded-sm"> {@html command.Description}</p>
    </summary>
    <div class="px-4 pb-2 prose prose-invert prose-code:bg-gray-700 before:prose-code:content-[''] after:prose-code:content-[''] prose-code:rounded-sm"> {@html command.HtmlDescription}</div>

    {#if command.GuildUserPermissions}
        <div class="mb-2">
            <h4 class="px-4 whitespace-nowrap text-ellipsis">User Guild Permissions</h4>
    ...
    <div>
        ...
        <ul class="list-none" aria-label="Command usage examples">
            {#each command.Example as example}
                <li>
                    <code class="text-sm bg-gray-700 ml-4 rounded-sm my-2 py-1">{truncateString(example, 70)}</code>
                </li>
            {/each}
        </ul>
    </div>
