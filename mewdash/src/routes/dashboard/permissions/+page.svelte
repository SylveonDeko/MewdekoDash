...
<script lang="ts">
  ...
  async function addPermissionOverride() {
    try {
      ...
      const newOverrideResponse = await api.addPermissionOverride(
        $currentGuild.id,
        {
      ...
      showNotificationMessage(
        "Failed to add permission override: " + error.message,
        "error",
      );
    }
  }
  ...
  async function updatePermissionOverride(override: PermissionOverride) {
    try {
      ...
      const updatedOverride = await api.addPermissionOverride(
        $currentGuild.id,
        {
      ...
      permissionOverrides = permissionOverrides.map((o) =>
        o.command === updatedOverride.command ? updatedOverride : o,
      );
      showNotificationMessage("Permission override updated", "success");
      editingOverride = null;
    } catch (error) {
      showNotificationMessage(
        "Failed to update permission override: " + error.message,
        "error",
      );
    }
  }
  ...
  async function deletePermissionOverride(commandName: string) {
    try {
      ...
      permissionOverrides = permissionOverrides.filter(
        (p) => p.command !== commandName,
      );
      showNotificationMessage("Permission override deleted", "success");
      editingOverride = null;
    } catch (error) {
      showNotificationMessage(
        "Failed to delete permission override: " + error.message,
        "error",
      );
    }
  }
  ...
  function togglePermission(permission: string) {
    ...
    if (editingOverride) {
      editingOverride.perm = BigInt(
        selectedPermissions.reduce(
          (acc, perm) => acc | PermissionParser.getPermissions()[perm],
          0,
        ),
      );
    } else {
      newOverride.perm = BigInt(
        selectedPermissions.reduce(
          (acc, perm) => acc | PermissionParser.getPermissions()[perm],
          0,
        ),
  ...
  function handleDropdownKeydown(event: KeyboardEvent) {
    if (event.key === "Escape") {
      ...
      const options = Array.from(
        dropdownRef.querySelectorAll('button[role="option"]'),
      );
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="relative">
          ...
          >
            ...
            <span
              class="transform transition-transform duration-200 {showPermissionDropdown
                ? 'rotate-180'
                : ''}"
            ...
            >
              {#each Object.entries(PermissionParser.getPermissions()) as [permission, value]}
                <button
                  ...
                  class="w-full text-left flex items-center p-2 hover:bg-gray-600 cursor-pointer {selectedPermissions.includes(
                    permission,
                  )
                    ? 'bg-green-700'
                    : ''}"
    ...
    {:else}
      <ul class="space-y-4" aria-label="Permission overrides list">
        {#each permissionOverrides as override (override.command)}
          <li transition:slide class="bg-gray-800 rounded-lg p-4">
            {#if editingOverride && editingOverride.command === override.command}
              <div class="flex flex-col gap-2">
                <label for="edit-command-{override.command}" class="sr-only"
                  >Command</label
                >
                <input
                    ...
                    >
                      {#each Object.entries(PermissionParser.getPermissions()) as [permission, value]}
                        <button
                          type="button"
                          role="option"
                          aria-selected={selectedPermissions.includes(
                            permission,
                          )}
                          on:click={() => togglePermission(permission)}
                          on:keydown={(event) =>
                            handleOptionKeydown(event, permission)}
                          class="w-full text-left flex items-center p-2 hover:bg-gray-600 cursor-pointer {selectedPermissions.includes(
                            permission,
                          )
                            ? 'bg-green-700'
              ...
              >
                <div>
                  ...
                  <p class="text-sm text-gray-400">
                    Permissions: {getPermissionNames(BigInt(override.perm))
