               ...
               border-color: {$colorStore.primary}30;"
                >
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h2 class="text-xl font-bold" style="color: {$colorStore.text}">Select Server</h2>
                    </div>

                    <div class="guild-selector relative" use:clickOutside on:clickoutside={closeDropdown}>
                        <button
                                class="dropdown-toggle w-full p-4 border rounded-lg cursor-pointer text-left flex items-center gap-3 transition-all duration-200"
                   ...
                   color: {$colorStore.text};"
                        >
                            {#if $currentGuild}
                                <img src={getGuildIconUrl($currentGuild)} alt=""
                                     class="guild-icon w-8 h-8 rounded-full object-cover"/>
                                <span class="guild-name flex-grow">{$currentGuild.name}</span>
                            {:else}
                                <div class="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center">
                                    <Settings class="w-5 h-5" style="color: {$colorStore.muted}"/>
                                </div>
                                <span class="guild-name flex-grow"
                                      style="color: {$colorStore.muted}">Select a Server</span>
                            {/if}
                            <div
                        ...
                        {#if dropdownOpen}
                            <ul
                                    class="dropdown-menu absolute top-full left-0 right-0 border rounded-lg mt-2 max-h-60 overflow-y-auto z-10 backdrop-blur-sm"
                     ...
                     border-color: {$colorStore.primary}30;"
                            >
                                {#each $userAdminGuilds as guild}
                                    <li
                                            role="option"
                                            aria-selected={$currentGuild === guild}
                                    ...
                                    >
                                        <button
                                                ...
                                                class="dropdown-item w-full text-left p-4 cursor-pointer transition-colors duration-200 flex items-center gap-3 hover:bg-opacity-20"
                                                style="background: {$currentGuild === guild ? $colorStore.primary + '20' : 'transparent'};
                           color: {$colorStore.text};"
                                        >
                                            <img
                                            ...
                                            {#if $currentGuild === guild}
                                                <Check class="ml-auto w-5 h-5" style="color: {$colorStore.primary}"/>
                                            {/if}
                                        </button>
                                    </li>
                                {/each}
