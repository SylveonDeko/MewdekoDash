...
<script lang="ts">
  ...
  } from "$lib/stores/animationStores.ts";

  let items = [
    { href: "/dashboard", text: "Dashboard", icon: "📊" },
    { href: "/dashboard/afk", text: "AFK", icon: "💤" },
    { href: "/dashboard/music", text: "Music", icon: "🎵" },
    ...
    { href: "/dashboard/multigreets", text: "MultiGreets", icon: "👋" },
  ];
  ...
  <div class="flex-grow flex {isMobile ? 'flex-col' : ''} overflow-hidden">
    {#if isMobile}
      <header class="bg-gray-800 p-4 flex items-center justify-between">
        ...
        <div class="text-white font-bold">
          {$currentGuild ? $currentGuild.name : "Dashboard"}
        </div>
        <div class="w-6"></div>
      </header>
    {/if}
    ...
    <aside
      class="sidebar {isMobile
        ...
        : 'w-64'} bg-gray-800 overflow-y-auto transition-transform duration-300 ease-in-out"
    ...
    >
      <div class="sidebar-content p-5">
        ...
        {/if}
        <div
          class="guild-selector relative mb-5"
          use:clickOutside
            ...
            {#key $currentGuild}
              {#if $currentGuild}
                <li>
                  <a
                    href="/dashboard"
                    class="nav-link block text-white no-underline p-2 mb-1 rounded transition-colors duration-200 hover:bg-gray-700 nav-link.active {$page
                ...
                {#each items.slice(1) as item, index}
                  <li>
                    {#key $triggerAnimation}
                      <a
                        href={item.href}
                        class="nav-link block text-white no-underline p-2 mb-1 rounded transition-colors duration-200 hover:bg-gray-700 {$page
    ...
    <main
      class="dashboard-content flex-grow p-4 overflow-y-auto transition-transform duration-300 ease-in-out"
